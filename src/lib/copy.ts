...
const copyByLocale: Record<Locale, LocalizedCopy> = {
  en: {
    locale: "en",
    siteTitle: "PixelHeic",
    siteSubtitle: "Convert Apple HEIC or HEIF photos to JPG, PNG, or WEBP directly in the browser.",
    ...
    status: {
      ready: "READY",
      metadataAvailable: "Exif keep",
      metadataLimited: "Limited",
      metadataStripped: "Stripped",
      metadataNone: "No meta",
    },
    dropzone: {
      title: "Drag, drop, or browse for HEIC / HEIF",
      subtitle: "Release to add Apple photos to the local queue",
      hint: "HEIC HEIF - up to 50MB each",
    ...
    settings: {
      ...
      localNoticeBody: "Files stay on the user's device by default. No server-side upload pipeline is required for conversion.",
      metadataNoticeTitle: "Metadata retention",
      metadataNoticeKeepJpg: "PixelHeic tries to write key EXIF back into JPG exports: shot time, camera, and GPS when available.",
      metadataNoticeKeepLimited: "PNG and WEBP exports do not guarantee full metadata retention in v1. PixelHeic only guarantees key EXIF write-back for JPG.",
      metadataNoticeStrip: "Strip mode exports clean files without writing source EXIF metadata back into the result.",
    },
    actions: {
    ...
    empty: {
      queue: "No HEIC photos in queue",
      downloads: "No completed output yet",
    },
    credits: {
      title: "Notes & credits",
      body: "Live Photo and multi-frame HEIC files export only the first still frame in v1. Built with browser-side HEIC decode, EXIF parsing, and ZIP tooling.",
    ...
    templates: {
      ...
      readyForConversion: "Ready for browser-side HEIC conversion.",
  ...
  zh: {
    locale: "zh",
    siteTitle: "PixelHeic",
    siteSubtitle: "把苹果 HEIC / HEIF 照片直接在浏览器里转成 JPG、PNG 或 WEBP。",
    queueTitle: "队列",
    ...
    dropzone: {
      ...
      subtitle: "松开即可把苹果照片加入本地队列",
      hint: "HEIC HEIF - 单个文件不超过 50MB",
    ...
    settings: {
      ...
      localNoticeBody: "默认不需要上传到服务端，文件直接在用户设备上完成解码、转换和下载。",
      metadataNoticeTitle: "元数据保留说明",
      metadataNoticeKeepJpg: "导出 JPG 时，PixelHeic 会尽量回写关键 EXIF：拍摄时间、设备信息和 GPS（如果源文件有）。",
      metadataNoticeKeepLimited: "v1 里 PNG 和 WEBP 不保证完整保留元数据。当前只对 JPG 做关键 EXIF 回写保证。",
      metadataNoticeStrip: "移除模式会导出更干净的结果文件，不再写回源文件 EXIF 元数据。",
    },
    actions: {
      convert: "开始转换",
      converting: "转换中...",
    ...
    credits: {
      title: "说明与致谢",
      body: "Live Photo 或多帧 HEIC 在 v1 里只导出第一张静态画面。当前方案基于浏览器端 HEIC 解码、EXIF 读取和 ZIP 打包。",
      chips: ["heic-to", "exifr", "piexifjs", "JSZip", "Vercel"],
    },
    templates: {
      unsupportedFileType: "不支持的文件类型：{{name}}",
      fileTooLarge: "文件过大（最大 {{maxMb}}MB）：{{name}}",
      largeFileWarning: "{{name}} 文件较大，当前设备上本地解码可能更慢。",
      ...
      readyForConversion: "已就绪，可在浏览器本地转换。",
      ...
      metadataLimitedPngWebp: "PNG 和 WEBP 在 v1 里可能无法完整保留源元数据。",
